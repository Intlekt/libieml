...
TEST(ieml_grammar_test_case, rootparadigm_has_good_range)
{
    std::ifstream file;
    file.open(std::string(TEST_IEML_GRAMMAR_EXAMPLES_FOLDER) + "/ieml-grammar.ieml");
    ...
    for (nlohmann::json::iterator it = res["elements"].begin(); it != res["elements"].end(); ++it)
    {
        ...
        if (v["type"] == "SCRIPT" && v["str"] == "E:M:O:.")
        {
            EXPECT_EQ(v["range"]["line_start"], 11);
        }
    }
}
...
TEST(ieml_grammar_test_case, has_phrase_words)
{
